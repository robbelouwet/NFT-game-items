...
contract("Collectible", (accounts) => {
	...
	describe("Setup tiers and blueprints testdata", async () => {
		it("Adding 3-8 tiers with buffer 3", async () => {
			...
			await collectible.addTier("normal", 0, 2);
			await collectible.addTier("common", 0, 5);
			await collectible.addTier("uncommon", 0, 20);
			await collectible.addTier("Legendary", 0, 50);
			await collectible.addTier("Exotic", 0, 100);
		});

		it('Add "Normal" items', async () => {
			// parameters: rarity of the tier, name, max instances (max supply)
			await collectible.addBlueprint(2, "Sword", 5);
			await collectible.addBlueprint(2, "Pickaxe", 5);
			await collectible.addBlueprint(2, "Axe", 5);
		});

		it('Add "Common" items', async () => {
			// parameters: rarity of the tier, name, max instances (max supply)
			await collectible.addBlueprint(5, "Sword", 5);
			await collectible.addBlueprint(5, "Pickaxe", 5);
			await collectible.addBlueprint(5, "Axe", 5);
		});

		it('Add "Uncommon" items', async () => {
			// parameters: rarity of the tier, name, max instances (max supply)
			await collectible.addBlueprint(20, "Sword", 5);
			await collectible.addBlueprint(20, "Pickaxe", 5);
			await collectible.addBlueprint(20, "Axe", 5);
		});

		it('Add "Legendary" blueprints', async () => {
			// parameters: rarity of the tier, name, max instances (max supply)
			await collectible.addBlueprint(50, "Flaming Sword", 10);
			await collectible.addBlueprint(50, "Shadow Dagger", 10);
			await collectible.addBlueprint(50, "Nexus", 10);
		});

		it('Add "Exotic" blueprints', async () => {
			// parameters: rarity of the tier, name, max instances (max supply)
			await collectible.addBlueprint(100, "Flaming Sword", 10);
			await collectible.addBlueprint(100, "Shadow Dagger", 10);
			await collectible.addBlueprint(100, "Nexus", 10);
		});
	});
	...
	describe("Tiers", async () => {
		it("Adding with rarity=0 fails", async () => {
			await tryCatch(
				collectible.addTier("Impossible", 0, 0),
				errTypes.revert
			);
		});
		...
		it("Adding duplicate tier masks fails", async () => {
			await tryCatch(
				collectible.addTier("normal", 0, 2),
				errTypes.revert
			);
		});
		...
		it("Adding duplicate tier names fails", async () => {
			await tryCatch(
				collectible.addTier("normal", 9, 3),
				errTypes.revert
			);
		});
		...
		it("Getting tier items count of nonexistent tier", async () => {
			await tryCatch(
				collectible.getTierBlueprintCount(25000),
				errTypes.revert
			);
		});
	});
	...
	describe("Blueprints", async () => {
		it("Getting max supply of nonexistent tier fails", async () => {
			await tryCatch(
				collectible.getBlueprintMaxSupply(25000, 0),
				errTypes.revert
			);
		});
		it("Getting max supply of nonexistent blueprint fails", async () => {
			await tryCatch(
				collectible.getBlueprintMaxSupply(3, 2500),
				errTypes.revert
			);
		});
		it("Adding blueprint with buff size overflowing max supply fails", async () => {
			...
			await tryCatch(
				collectible.addBlueprint(3, "Dagger", 3),
				errTypes.revert
			);
		});
		it("Adding existing blueprint fails", async () => {
			await tryCatch(
				collectible.addBlueprint(5, "Sword", 5),
				errTypes.revert
			);
		});
	});
	...
	describe("Buy ticket", async () => {
		it("Buy 1 tickets and receive change back.", async () => {
			const amount = 1;
			const balance_before = await web3.eth.getBalance(
				collectible.address
			);

			const tx = await collectible.buyTicket.sendTransaction(
				"appel",
				amount,
				{
					from: accounts[0],
			...
			const balance_after = await web3.eth.getBalance(
				collectible.address
			);
			const ticket_price = await collectible.getTicketPrice();
			assert.equal(
		...
		it("Pop the only ticket we have", async () => {
			...
			const tx = await collectible.popTicket(accounts[0]);
		});

		it("Pop a ticket we don't have", async () => {
			//await Promise.resolve(() => setTimeout(5000))
			await tryCatch(collectible.popTicket(accounts[0]), errTypes.revert);
		});

		it("Buy 2 tickets with insufficient funds", async () => {
			const amount = 2;
			const tx = async () =>
				await collectible.buyTicket.sendTransaction("appel", amount, {
					from: accounts[0],
